...
def create_FCN(filter_list, k_size_list,conv_activation,
                 ...
                 summary = True):
    '''
    Standard FCN model frm the paper "Strong baseline" adding dropout and regularization.
    '''
...
def create_FCN_dilation(filter_list, k_size_list,conv_activation,
                 dense_activation, window_size, n_features, output_size,optimizer, summary = True):
    '''
    Modification of the base model by adding a dilation rate with a factor of 2 per layer. This idea is taked from the
...
def create_FCN_2_1(filter_list, k_size_list,dense_activation, window_size, 
    ...
    '''
    FCN model with regularization and 2:1 ConvNet ReLU ratio
    '''
    x = Input(shape=(window_size,n_features))
    ############# First block ################
    block_1 = BatchNormalization(axis = 1)(x)
...
def create_FCN_leaky_ensemble(filter_list, k_size_list,conv_activation,
                 dense_activation, window_size, n_features, output_size,optimizer, summary = True):
    '''
    FCN model with regularization and a Leaky ReLU ensemble architecture
    '''
    x = Input(shape=(window_size,n_features))
    ############# First block ################
    block_1 = BatchNormalization(axis = 1)(x)
...
def FCN_reg(filter_list, k_size_list,conv_activation,
                 dense_activation, window_size, n_features,optimizer, summary = True):
     
    x = Input(shape=(window_size,n_features))
    ## First block
    block_1 = BatchNormalization(axis = 1)(x)
    block_1 = Conv1D(padding='same',activation =  conv_activation,
                            kernel_size = k_size_list[0], filters= filter_list[0])(x)
    block_1 = BatchNormalization(axis = 1)(block_1)
    
    ## Second block
    block_2 = Conv1D(padding='same',activation =  conv_activation,
                            kernel_size = k_size_list[1], filters= filter_list[1])(block_1)
    block_2 = BatchNormalization(axis = 1)(block_2)
    
    ## Third block
    block_3 = Conv1D(padding='same',activation =  conv_activation,
                            kernel_size = k_size_list[2], filters= filter_list[2])(block_2)
    block_3 = BatchNormalization(axis = 1)(block_3)
    
    ## Final layers
    full = GlobalAveragePooling1D()(block_3)  
    output = Dense(1, activation= dense_activation)(full)
